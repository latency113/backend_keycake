...
const ResponseSchema = z.object({
  data: CakeCountOptionalDefaultsSchema.array(),
  message: z.string(),
  meta_data: z.object({
  ...
  app.get(
    ...
    async ({ query, set }) => {
      try {
        ...
        const [result, total] = await Promise.all([
          deps.CakeCountService.getAll({ pagination: { limit, page } }),
          deps.CakeCountService.count(),
        ]);
        if (!result)
          throw NewError("Failed to fetch CakeCount", "FETCH_FAILED", 500);
        ...
        console.error("Error fetching CakeCountType:", error);
    ...
    },
    {
      detail: {
        ...
        responses: {
          200: {
            description: "CakeCount fetch success",
            content: {
              "application/json": {
          ...
          500: {
            description: "CakeCount fetch fail",
            content: {
              "application/json": {
